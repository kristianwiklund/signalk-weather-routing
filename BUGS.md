...
| # | Description |
|---|---|
| [BUG-136](https://github.com/kristianwiklund/signalk-weather-routing/issues/382) | The legacy wind/wave GRIB path and the new generic GRIB engine return different wave heights at the northern edge of the Baltic_South ICON_EU_EWAM fixture (lat≈59.03, lon 18–22°E). At that row the legacy path returns `undefined` for all sampled times and longitudes; the new generic engine returns wave heights of 0.07–0.53 m. Surfaced 2026-06-21 by the REQ-141 Phase A differential harness. |
| [BUG-137](https://github.com/kristianwiklund/signalk-weather-routing/issues/383) | The legacy ocean-current GRIB path and the new generic GRIB engine return different current vectors near land in the Current_ba fixture (lat≈63.41, lon≈21.67, Bothnian Sea). At that point the legacy path returns a non-zero current (e.g. `{u:-0.17, v:0.01}`); the new generic engine returns `{u:0, v:0}`. Surfaced 2026-06-21 by the REQ-141 Phase A differential harness. |
