...
| # | Description |
|---|---|
| [BUG-136](https://github.com/kristianwiklund/signalk-weather-routing/issues/382) | The legacy wind/wave GRIB path and the new generic GRIB engine return different **significant wave height (swh, GRIB HTSGW)** values at the northern edge of the Baltic_South ICON_EU_EWAM fixture. At lat=59.03125 (59°01'52.500"N), lon ∈ {18.36875 (18°22'07.500"E), 20.16875 (20°10'07.500"E), 21.96875 (21°58'07.500"E)}, for each of the three sampled times (2026-06-06T00:00:00Z, 2026-06-08T12:00:00Z, 2026-06-11T00:00:00Z), the legacy path returns `undefined`; the new generic engine returns swh in the range 0.07–0.53 m (e.g. 0.1832246845588078 m at lon=18.36875, t=2026-06-06T00:00:00Z). Surfaced 2026-06-21 by the REQ-141 Phase A differential harness. **Investigation (2026-06-21):** the divergent coordinates are OUTSIDE the wave grid. The Baltic_South fixture has two grids: wind grid lat [52.53125, 59.03125] step 0.0625°; wave grid (swh, identical in both legacy and new paths) lat [52.5, 59.0] step 0.05° nLat=131. The divergent latitude 59.03125 is the wind grid's latMax but is 0.03125° NORTH of the wave grid's latMax (59.0). The legacy path correctly returns `undefined` (coordinate-based bounds check: `lat > latMax`). The new engine incorrectly returns a value because `sampleGrid` checks bounds by index (`i0 >= nLat`) rather than by coordinate: for lat=59.03125, latF=130.625, i0=130, and 130 < 131 (nLat) passes, so the point is accepted and the interpolation degenerates to the last row's value. The legacy `undefined` is the correct answer; the new engine has a bounds-check defect. |
| [BUG-137](https://github.com/kristianwiklund/signalk-weather-routing/issues/383) | The legacy ocean-current GRIB path and the new generic GRIB engine return different current vectors near land in the Current_ba fixture. At lat=63.41000000000001 (63°24'36.000"N), lon=21.6665305 (21°39'59.510"E) (Bothnian Sea), for each of the three sampled times, the legacy path returns a current of magnitude ~0.15–0.17 m/s flowing toward ~273°/292°/104° (WSW/WNW/ESE); the new generic engine returns magnitude 0 m/s (calm). Exact values: t=2026-06-14T00:15:00Z legacy mag 0.1689 m/s flow 273.0°; t=2026-06-14T12:15:00Z legacy mag 0.1564 m/s flow 292.0°; t=2026-06-15T00:00:00Z legacy mag 0.1552 m/s flow 104.1°. Surfaced 2026-06-21 by the REQ-141 Phase A differential harness. |
...
| [~~BUG-121~~](https://github.com/kristianwiklund/signalk-weather-routing/issues/324) | ~~closestPoint reduce pattern repeated 3x.~~ — **fixed** (extracted `closestTo()` helper; dead instance removed in BUG-113; confirmed 2026-06-15) |
| [~~BUG-130~~](https://github.com/kristianwiklund/signalk-weather-routing/issues/344) | ~~CI fails — eslint+prettier cleanup needed.~~ — **fixed** (all source files formatted with prettier; eslint errors fixed; accepted `any` usages documented with inline disables; 0 errors 0 warnings; confirmed 2026-06-19) |
| [~~BUG-107~~](https://github.com/kristianwiklund/signalk-weather-routing/issues/310) | ~~SignalK app object typed as any everywhere.~~ — **fixed** (defined `SignalKApp` interface in `signalk-app.ts`; applied to `index.ts`, `setup.ts`, `resources.ts`; confirmed 2026-06-15) |
| [~~BUG-117~~](https://github.com/kristianwiklund/signalk-weather-routing/issues/320) | ~~Frontend innerHTML with unescaped external data.~~ — **fixed** (added `escapeHtml()` helper; applied to 8 innerHTML assignments; confirmed 2026-06-16) |
| [~~BUG-58~~](https://github.com/kristianwiklund/signalk-weather-routing/issues/188) | ~~interpolateBoatSpeed clamps to min TWS column below polar minimum.~~ — **fixed** (linear interpolation toward zero: `speed = polar_min_speed × (TWS / polar_min_TWS)`; preserves frontier points at 4-5 kn TWS; avoids Ålandförträngningen regression; confirmed 2026-06-19) |
| [~~BUG-133~~](https://github.com/kristianwiklund/signalk-weather-routing/issues/348) | ~~Region avoidance checkbox label click blinks then reverts.~~ — **fixed** (removed redundant click handler that double-toggled; native `<label>` behavior + `change` event handler is sufficient; confirmed 2026-06-19) |
| [~~BUG-128~~](https://github.com/kristianwiklund/signalk-weather-routing/issues/334) | ~~Isochrones drawn during routing even when checkbox unticked.~~ — **fixed** (progress handler now checks `isochrone-toggle.checked` before adding polylines; confirmed 2026-06-19) |
| [~~BUG-83~~](https://github.com/kristianwiklund/signalk-weather-routing/issues/258) | ~~'No grib files cover departure time' error misleading when only current GRIB loaded.~~ — **fixed** (error message now says 'No wind GRIB files cover the requested departure time'; confirmed 2026-06-19) |
| [~~BUG-134~~](https://github.com/kristianwiklund/signalk-weather-routing/issues/353) | ~~Wind barbs show calm despite valid wind data.~~ — **fixed** (root cause: `rebuildScrubberTimes()` reconstructed time axis with even spacing; ICON-EU has variable spacing (hourly→3-hourly); divergence reached 12+ hours at idx 38; overlay showed wind from wrong time step; fixed by using actual times from `/wind-times` endpoint; also resampled wind at each waypoint's own position in backtrack; confirmed 2026-06-19) |
| [~~BUG-132~~](https://github.com/kristianwiklund/signalk-weather-routing/issues/347) | ~~Scrubber grib color bar shows stale render after route locks range.~~ — **fixed** (extracted `renderCoverageBar(rangeStart, rangeEnd)`; coverage bar now rebuilds on route lock with clamped partial overlaps; added Full/Route range toggle button; confirmed 2026-06-19) |
| [~~BUG-131~~](https://github.com/kristianwiklund/signalk-weather-routing/issues/346) | ~~savePluginOptions drops gribDir and polarPath from config.~~ — **fixed** (passed full `settings` object instead of partial `{ avoidRegionIds }`; SignalK API overwrites entire configuration key; confirmed 2026-06-19) |
| [~~BUG-116~~](https://github.com/kristianwiklund/signalk-weather-routing/issues/319) | ~~loadRegions mutates plugin config during read operation.~~ — **fixed** (auto-clean extracted to `cleanStaleRegionIds()`, called only from `start()`; confirmed 2026-06-16) |
| [~~BUG-115~~](https://github.com/kristianwiklund/signalk-weather-routing/issues/318) | ~~Grid iteration logic duplicated 3x across grid endpoints.~~ — **fixed** (extracted `computeGridBounds()` helper + 4 unit tests; confirmed 2026-06-16) |
| [~~BUG-119~~](https://github.com/kristianwiklund/signalk-weather-routing/issues/322) | ~~Inconsistent import fs style across codebase.~~ — **fixed** (standardised to `node:` prefix in polar.ts, setup.ts, polar.test.ts; confirmed 2026-06-15) |
| [~~BUG-118~~](https://github.com/kristianwiklund/signalk-weather-routing/issues/321) | ~~dilate.ts is dead code with stale comment.~~ — **fixed** (deleted dilate.ts and dilate.test.ts; dilation is handled by Python build script; confirmed 2026-06-15) |
| [~~BUG-114~~](https://github.com/kristianwiklund/signalk-weather-routing/issues/317) | ~~Redundant double coverage check in /wave-grid.~~ — **fixed** (merged two `loaded.some(...)` checks into one; confirmed 2026-06-15) |
| [~~BUG-113~~](https://github.com/kristianwiklund/signalk-weather-routing/issues/316) | ~~Dead reduce on empty isochrone array in failure path.~~ — **fixed** (removed dead reduce and misleading "closest approach: 0 nm" from error message; confirmed 2026-06-15) |
| [~~BUG-108~~](https://github.com/kristianwiklund/signalk-weather-routing/issues/311) | ~~buildRegionIndex MultiPolygon key generation inconsistent.~~ — **fixed** (uses per-UUID `ringIdx` counter; all keys now consistently `id__0`, `id__1`, etc.; confirmed 2026-06-15) |
| [~~BUG-110~~](https://github.com/kristianwiklund/signalk-weather-routing/issues/313) | ~~DEG_TO_RAD constant duplicated ~10 times.~~ — **fixed** (exported `DEG_TO_RAD` and `RAD_TO_DEG` from `geo.ts`; replaced 12 inline `Math.PI / 180` across `geo.ts` and `isochrone.ts`; confirmed 2026-06-15) |
| [~~BUG-106~~](https://github.com/kristianwiklund/signalk-weather-routing/issues/309) | ~~Widespread as any casts for gdal-async.~~ — **fixed** (centralised in `readBandPixels`, `vsimemCopy`, `vsimemRelease` typed wrappers; 8 inline casts removed; confirmed 2026-06-15) |
| [~~BUG-111~~](https://github.com/kristianwiklund/signalk-weather-routing/issues/314) | ~~console.log SSE debugging calls in index.ts.~~ — **fixed** (3 calls removed — SSE hang they debugged is resolved; confirmed 2026-06-15) |
| [~~BUG-104~~](https://github.com/kristianwiklund/signalk-weather-routing/issues/307) | ~~getWaveAt doesn't validate lat/lon bounds before bilinear.~~ — **fixed** (bounds check added using swhGrid params; returns `undefined` for out-of-grid points; confirmed 2026-06-15) |
| [~~BUG-103~~](https://github.com/kristianwiklund/signalk-weather-routing/issues/306) | ~~loadRegions savePluginConfig fire-and-forget without await.~~ — **fixed** (`await` added; try/catch now catches async rejections; confirmed 2026-06-15) |
| [~~BUG-97~~](https://github.com/kristianwiklund/signalk-weather-routing/issues/295) | ~~Optional chaining on polar speed lookups on every hot-path call.~~ — **fixed** (direct indexing after parsePolar guarantees array shape; confirmed 2026-06-15) |
| [~~BUG-96~~](https://github.com/kristianwiklund/signalk-weather-routing/issues/294) | ~~console.log on isochrone hot path without debug guard.~~ — **fixed** (`process.env.DEBUG` guard added to `logStepTiming`/`logTimingSummary`; prevents eager string allocation; confirmed 2026-06-15) |
| [~~BUG-94~~](https://github.com/kristianwiklund/signalk-weather-routing/issues/292) | ~~Current drift longitude uses newLat for cosine correction.~~ — **fixed** (uses `point.lat` instead of `newLat`; confirmed 2026-06-15) |
| [~~BUG-102~~](https://github.com/kristianwiklund/signalk-weather-routing/issues/305) | ~~Invalid departureTime produces misleading "No GRIB files cover" error.~~ — **fixed** (`isNaN(departureMs)` check added; returns "Invalid departureTime — expected ISO 8601 string"; confirmed 2026-06-15) |
| [~~BUG-101~~](https://github.com/kristianwiklund/signalk-weather-routing/issues/304) | ~~getWave has same silent fallback as BUG-93 for wave data.~~ — **fixed** (fallback levels removed; returns `undefined` when no file covers both spatially and temporally; confirmed 2026-06-15) |
| [~~BUG-100~~](https://github.com/kristianwiklund/signalk-weather-routing/issues/303) | ~~Waypoint GRIB coverage check gated on land avoidance.~~ — **fixed** (coverage check moved outside `useLandAvoidance` guard; always checked regardless of land avoidance setting; confirmed 2026-06-15) |
| [~~BUG-99~~](https://github.com/kristianwiklund/signalk-weather-routing/issues/302) | ~~`/avoid-regions` accepts all UUIDs when no regions loaded.~~ — **fixed** (returns HTTP 400 when `valid.size === 0`; confirmed 2026-06-15) |
| [~~BUG-95~~](https://github.com/kristianwiklund/signalk-weather-routing/issues/293) | ~~Silent GRIB load failures during calculation.~~ — **fixed** (failures tracked in `calcFailedFiles`, surfaced as warning in SSE done/warning event; `console.warn` replaced with `app.debug` (also fixes BUG-105); confirmed 2026-06-15) |
| [~~BUG-93~~](https://github.com/kristianwiklund/signalk-weather-routing/issues/291) | ~~`selectFile` silent fallback returns wrong-file data.~~ — **fixed** (fallback levels removed; `getWind` guards with `coversPointAtTime`, returns `{u:0,v:0}` when uncovered; `getFilePathForPoint` returns empty string; confirmed 2026-06-15) |
| [~~BUG-92~~](https://github.com/kristianwiklund/signalk-weather-routing/issues/290) | ~~Seed lat/lon validation rejects valid 0° coordinates.~~ — **fixed** (extracted `validateCalculateInput` using `typeof` check instead of falsy; 6 unit tests added; confirmed 2026-06-15) |
| [~~BUG-98~~](https://github.com/kristianwiklund/signalk-weather-routing/issues/300) | ~~The hires variants of the GSHHG data looked identical to the normal (h variant).~~ — **fixed** (`prepare-land-data.py` now accepts `-r f`; genuine f-tier binaries built and uploaded as release assets to `weather-routing-hires-land-data` v1.0.0; installed in test container and "(hires)" label confirmed visible; confirmed 2026-06-15) |
...
| [~~BUG-72~~](https://github.com/kristianwiklund/signalk-weather-routing/issues/216) | ~~When `windSpeedMs` is enabled, the conditions graph dual-axis layout is broken: the additional outer left axis for TWS in m/s is missing, and the overall graph layout is visually incorrect.~~ — **fixed** (added ml=60 left margin in dual-axis mode; axis labels live in the margin, data starts at x=ml; boat-speed axis right-aligned to margin boundary; legend and GRIB stripe label shifted with ml; confirmed 2026-06-13) |
| [~~BUG-63~~](https://github.com/kristianwiklund/signalk-weather-routing/issues/221) | ~~Wave overlay does not disappear when GRIB file is unticked.~~ — **fixed** (GRIB toggle now calls `fetchWavePoints`; zero-enabled-files case short-circuits to clear overlay locally, bypassing the backend no-filter behaviour; confirmed 2026-06-13) |
| [~~BUG-71~~](https://github.com/kristianwiklund/signalk-weather-routing/issues/212) | ~~Unit preferences not applied in the frontend.~~ — **fixed** (`.categories` nesting fix, `displayFormat` precision fix, safety margin label converted to active distance unit; confirmed 2026-06-13. Note: `/signalk/v1/unitpreferences/active` returns the server-wide default only — per-user preference changes require setting "default for new users" in SignalK, which is SignalK API behaviour) |
