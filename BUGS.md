...
| # | Description |
|---|---|
| [BUG-53](https://github.com/kristianwiklund/signalk-weather-routing/issues/131) | Route from Öregrund to Helsinki (BUG-51 acceptance test, REQ-73 deployed) reaches the destination but with excessive wandering outside Estonia before arriving. **Investigation (2026-06-07):** Added `coneDisabled` counter to step logging. From step 2 onwards, 76–99% of all frontier points have their cone disabled — the REQ-73 land check tests the full frontier→destination segment (~230 nm). From any point in the Baltic, this segment crosses the Finnish/Estonian coast, so nearly every point gets `directPathBlockedByLand=true` and is given a full 180° cone. The algorithm runs with effectively no directional constraint for the entire route. **Fix:** Cap the land check at 100 nm lookahead along the bearing to destination. Points far from the destination only check the first 100 nm of the direct bearing for land — which stays in open water from the Baltic. Points within 100 nm still check the full segment to destination. This preserves the BUG-51 fix (Swedish archipelago escape) since those frontier points are within 20–30 nm of coastal land. |
