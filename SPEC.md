...
| # | Requirement | Status |
|---|---|---|
| [REQ-102](https://github.com/kristianwiklund/signalk-weather-routing/issues/NEW) | log it as a separate feature request | open |
| | **Interpretation:** The agent implemented a wave height colour raster overlay on the map without explicit go-ahead on the implementation step. This entry documents that implementation as a separate feature request: a canvas-based raster overlay (one pixel per GRIB grid cell, bilinear interpolation via browser image scaling) rendering HTSGW data with a blue-to-red gradient, configurable max height (`waveOverlayMaxM`, default 3.0 m), transparent below 0.2 m, with a legend in the bottom-right corner. The overlay is driven by the same time scrubber and file checkboxes as the wind overlay, and re-renders on map pan/zoom. | pending |
| [REQ-101](https://github.com/kristianwiklund/signalk-weather-routing/issues/195) | A transparent overlay that puts color on top of the map. No color below 20 cm, then a scale from blue to red, where red is configurable in the plugin config. | open |
| | **Interpretation:** A Leaflet overlay layer (similar to the existing wind GRIB overlay) that renders wave height data from the loaded GRIB as a semi-transparent colour raster on the map. Colours are: transparent for wave heights < 0.2 m, then a blue-to-red colour gradient, with the upper bound of the gradient (the "red" end) being a user-configurable plugin setting (default value TBD). The overlay takes HTSGW (or equivalent wave height) data from the GRIB files and renders it per-pixel using the same time-scrubber driven time-step selection as the wind overlay. | pending |
...
| [REQ-93](https://github.com/kristianwiklund/signalk-weather-routing/issues/171) | A pre-flight advisory check estimates whether the loaded GRIB forecast covers the full passage before routing starts. The estimate is: haversine straight-line distance from start to destination ÷ a configurable expected average speed (new input field, default 5 kn) = estimated passage hours. If departure_time + estimated_hours exceeds the last GRIB timestep, a warning is shown before calculation starts, using the same pre-flight pattern as the existing departure-outside-GRIB and start-on-land checks. The warning is advisory (not a hard block), because the actual route may differ from the straight-line estimate. | open |
| [REQ-88](https://github.com/kristianwiklund/signalk-weather-routing/issues/165) | Each GRIB file entry in the sidebar shows the model run timestamp (GRIB reference time) and its age in hours. Files older than 12 hours are visually flagged with an amber indicator; files older than 24 hours with a red indicator. | open |
| [REQ-89](https://github.com/kristianwiklund/signalk-weather-routing/issues/166) | A "Use vessel position" button on the routing page sets the start point to the vessel's current GPS position by reading `navigation.position` from the SignalK WebSocket/REST API. The button is disabled (greyed out) when the vessel position is not available. | open |
...
| [REQ-91](https://github.com/kristianwiklund/signalk-weather-routing/issues/168) | When the calculated route passes through Öresund, the Skagerrak approaches, or the German Bight, the UI displays a prominent warning that tidal currents are not modelled and may significantly affect ETA and conditions in that region. | open |
