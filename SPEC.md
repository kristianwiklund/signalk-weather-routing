...
| ~~REQ-35~~ | ~~During the coarse pre-pass, after each frontier pruning step, discard any frontier point from which the destination cannot be reached within the remaining GRIB forecast period even at maximum polar speed.~~ Superseded by REQ-36: the time-based cone (using GRIB end as deadline) was functionally inert — with 77+ remaining hours, points would need to be 600+ NM from the destination before being pruned. REQ-36's bearing-based cone replaces it. | superseded |
| REQ-36 | The map only draws frontier points that have passed all pruning steps. Points that survive sector pruning but are subsequently eliminated by T_bound or cone pruning must not appear in the drawn isochrone lines. Both the coarse pre-pass and the fine pass apply a bearing-based cone (±120° of bearing from start to destination): coarse pass for expansion and drawing; fine pass for drawing only (expansion remains T_bound-filtered to preserve admissibility). | done |
