...
| REQ-38 | Each isochrone calculation step emits a structured timing breakdown: number of frontier points, number of candidates evaluated, number of land checks performed, time spent in wind lookups, polar lookups, land checks, and frontier pruning. The breakdown is logged per step and summarised (min/max/total) at the end of the calculation. | open |
