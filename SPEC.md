...
| [REQ-73](https://github.com/kristianwiklund/signalk-weather-routing/issues/126) | The fine-pass directional cone is applied conditionally per frontier point: if the straight-line segment from the frontier point to the destination crosses land, the cone is disabled for that point (all 360° of headings are tried); if the segment is clear, the normal cone applies. This allows the frontier to escape start positions where land lies on the direct bearing to the destination, while still preventing backward routing once the boat is in open water with a clear path. | open |
