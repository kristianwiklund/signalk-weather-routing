...
| # | Requirement | Status |
|---|---|---|
| [REQ-100](https://github.com/kristianwiklund/signalk-weather-routing/issues/187) | **Original:** "the problem is that the barbs as they are drawn now do not look like arrows. the ring looks like an anchor point from where the arrow emerges. The easiest way to fix this is to remove the rings on non-zero barbs. Calm is a ring with a dot, everything else need to be a barb without a ring. Here, the problem is that winds below 5 kn still are sailable but not visible in a standard barb. SOlution: Ring and dot for very low wind speed (below polar cutoff as in config), put an arrowhead on the barb for barbs and point it towards where the wind is blowing" **Interpretation:** Wind below `polar.tws[0]` (the polar's minimum measured TWS column, e.g. 6 kn) → calm symbol (ring with centre dot, no staff). Wind at or above `polar.tws[0]` → directional barb: existing Beaufort ticks/pennants, ring removed, arrowhead added at the downwind (TOWARD) tip of the staff. `polar.tws[0]` is used as the threshold because below it the polar has no measured data — BUG-36 and BUG-58 document that the current code clamps to the minimum-column speed in this range, producing physically incorrect boat speeds (e.g. 6 kn boat speed at 3 kn wind). Applies to all three barb types: route waypoint barbs, leg-midpoint barbs, and GRIB overlay barbs. `polar.tws[0]` is passed from the server to the frontend alongside other polar metadata. | open |
