...
| [REQ-92](https://github.com/kristianwiklund/signalk-weather-routing/issues/169) | The user can place one or more intermediate waypoints on the map that the calculated route must pass through in order. The routing algorithm calculates the optimal path from start through each intermediate waypoint in sequence to the destination. | open |
