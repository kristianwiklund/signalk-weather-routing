...
| # | Requirement | Status |
|---|---|---|
| [REQ-76](https://github.com/kristianwiklund/signalk-weather-routing/issues/132) | The webapp has three clearly labelled test buttons: (1) the existing "Run Test" button reverted to start at Öregrund (60.3996°N, 18.3403°E) with departure 2024-05-24 08:00 local as currently configured; (2) a new "Helsinki Test" button pre-filling start Öregrund (60.3996°N, 18.3403°E), destination Helsinki (60.0902°N, 24.9472°E), departure at the start of the June 6 GRIBs; (3) a new "Gothenburg Test" button pre-filling start Öregrund (60.3996°N, 18.3403°E), destination Gothenburg (57.6138°N, 11.5980°E), departure at the start of the June 6 GRIBs. All three buttons are clearly distinguished in the UI. | open |
...
| [REQ-69](https://github.com/kristianwiklund/signalk-weather-routing/issues/120) | Remove the coarse pass (`runCoarsePass`) and T_bound pruning from the isochrone algorithm. **Rationale:** The coarse pass was introduced (REQ-34) to allow the fine pass to prune provably-slow frontier points using a T_bound upper-bound arrival time; the original performance justification was that land checks were expensive and T_bound caused the frontier to shrink, reducing total land-check count. REQ-41 (edge-tile index) made land checks sub-millisecond, removing that bottleneck. The coarse pass runs 18 headings/step with no cone and no heading-change constraint; for narrow-approach destinations (e.g. Öregrund→Gothenburg) it cannot land within the 2 nm arrival radius and returns null — paying ~1M iterations of overhead with no benefit. The per-position cone (BUG-43 fix) provides directional containment of the frontier, replacing T_bound's main role in preventing omnidirectional expansion. Removing the coarse pass halves routing time for failure cases and simplifies progress reporting to a single 0–100% pass. | open |
