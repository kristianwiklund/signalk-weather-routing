...
| # | Requirement | Status |
|---|---|---|
| [REQ-95](https://github.com/kristianwiklund/signalk-weather-routing/issues/175) | The conditions graph widget spans the same horizontal time range as the time scrubber. When a route is calculated, both the scrubber and the graph start at the route departure time and end at the estimated arrival time, so their horizontal extents are visually aligned — dragging the scrubber to any position corresponds to the same point in time on the graph. | open |
