...
| [REQ-91](https://github.com/kristianwiklund/signalk-weather-routing/issues/168) | **Original:** «When the calculated route passes through Öresund, the Skagerrak approaches, or the German Bight, the UI displays a prominent warning that tidal currents are not modelled and may significantly affect ETA and conditions in that region.»<br>**Revised interpretation:** The original regional-warning approach is an assumption violation — the plugin is global and hardcoding European bounding boxes is wrong. The correct solution is to support ocean current GRIB files (e.g. RTOFS or OpenWRF) as an optional data source. The routing algorithm adds interpolated ocean current U/V vectors to the boat's velocity through water, giving correct ground-track advancement globally. When a current GRIB is loaded, currents are applied automatically; no region-specific warnings are needed. Open questions: (1) auto-detect current GRIBs in `gribDir` or separate config path? (2) GRIB2 parameter IDs for ocean U/V current vs. wind U/V? (3) UI indicator when currents are active? | open |
