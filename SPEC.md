...
| REQ-37 | The webapp has a "Run test" button that pre-fills start (60.3996°N 18.3403°E), finish (58.5052°N 17.3474°E), and departure time (2026-05-24 10:30 CEST = 08:30 UTC) and immediately starts a routing run. A command-line script invokes the same test run with the same fixed parameters. | done |
...
| REQ-46 | The webapp shows a graph of forecasted conditions along the calculated route over time: wind speed, wind direction, and wave height (if wave data is available) at each waypoint's estimated arrival time. | done |
