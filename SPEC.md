...
| [REQ-141](https://github.com/kristianwiklund/signalk-weather-routing/issues/373) | **Original:** «We need true multi file support EXACTLY as for wind gribs. … gribs are spread in both time and space. For longer journeys it is necessary to combine several different physical files for coverage. This is a generic requirement for all types of valid input data.»<br>**Interpretation:** All valid GRIB input types — wind, ocean current, wave, and future variables (wave direction, etc.) — support multi-file combination across both time and space, by the same mechanism. A GRIB file is treated as a set of named channels; a generic engine combines files per channel (ranking overlaps by referenceTime → granularity → spatial → mtime). Makes current multi-file (resolves BUG-135) and standalone wave (REQ-140) first-class rather than special cases, and eliminates the wind/current loader+provider duplication. Design and phased plan in `docs/generic-grib-engine.md`. | open |
