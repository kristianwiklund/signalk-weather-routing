...
| [REQ-82](https://github.com/kristianwiklund/signalk-weather-routing/issues/152) | Rename the existing `minBoatSpeed` tunable to reflect its new role: a threshold below which polar speed is treated as zero rather than causing the candidate to be immediately discarded. When polar speed is below this threshold, it is set to zero; the discard is not automatic — what happens next depends on other logic (e.g. motor fallback). | open |
...
| [REQ-84](https://github.com/kristianwiklund/signalk-weather-routing/issues/154) | The motoring UI on the routing page is replaced with a two-field group: "Motor below _ kn" (the wind/boat-speed threshold that triggers motoring) and "Motor speed _ kn" (the speed used when motoring). The two fields are presented together as a coherent motor configuration. | open |
