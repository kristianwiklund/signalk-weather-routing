...
| [REQ-82](https://github.com/kristianwiklund/signalk-weather-routing/issues/152) | When polar-computed boat speed is below the `minBoatSpeed` threshold, the effective speed for that heading is set to zero rather than the heading being immediately discarded. Subsequent logic then determines the outcome: REQ-84 (motor) or REQ-83 (wait for wind) may act on the zero-speed candidate; if neither applies, the candidate is discarded. | open |
